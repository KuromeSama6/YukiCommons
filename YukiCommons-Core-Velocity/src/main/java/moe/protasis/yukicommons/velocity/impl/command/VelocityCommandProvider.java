    ...
    private static class Handler implements RawCommand {
        ...
        public void execute(Invocation invocation) {
            ...
            try {
                ...
                commandSender.sendMessage(Component.text("Permission denied"));

                YukiCommonsVelocity.getInstance().GetLogger().warning(String.format("Permission denied for %s whilst executing command %s:", commandSender, args));
                e.printStackTrace();

            } catch (CommandExecutionException e) {
                commandSender.sendMessage(Component.text("The command was not executed correctly."));
        ...
        public boolean hasPermission(Invocation invocation) {
            return invocation.source().hasPermission(handler.GetPermission());
        }
    }
}
